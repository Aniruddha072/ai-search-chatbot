...
async def test_partial_failure_returns_results_from_successful_queries():
    ...
    orchestrator = SearchOrchestrator(provider=provider, timeout_seconds=5.0)
...
async def test_slow_query_is_dropped_after_timeout():
    ...
    orchestrator = SearchOrchestrator(provider=provider, timeout_seconds=0.05)
...
async def test_queries_run_concurrently_not_sequentially():
    ...
    orchestrator = SearchOrchestrator(provider=UniformDelayProvider(), timeout_seconds=5.0)
