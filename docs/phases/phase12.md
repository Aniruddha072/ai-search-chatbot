...
- `src/config/settings.py` / `.env.example`: `evaluation_timeout_seconds` raised 15.0 -> 30.0 - the old budget was tighter than a real successful call's own measured runtime, before any rate-limit retry gets added on top. Considered and rejected switching `RagasEvaluator`'s model for more per-minute headroom: probed real rate-limit headers for both Groq models and found the smaller `llama-3.1-8b-instant` (6,000 TPM) has *less* headroom than the `llama-3.3-70b-versatile` model already in use (12,000 TPM) - switching would have made it worse.
